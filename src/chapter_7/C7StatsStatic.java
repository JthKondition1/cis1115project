...
/**
 * Sum up the numbers of given array.
 * @param x 
 * @return
 */
...
/*
        double sum = 0; 
        double varSq = 0; //sum of squared numbers for variance
        int count = 0;
        //double varTemp = 0;
        //double variance = 0;
        double standardD = 0;
        for (int i = 0; i < x.length; i++) { // Gets average; variance, square root of var;
            sum += x[i]; 
            count++;
            double average = sum / count; // average 
            varSq += Math.pow(x[i] - average,2); // variance 
            //double variance = varSq / (count-1);
           // double variance = varTemp / (count-1);
          //  standardD = Math.sqrt( ( Math.pow(sum-average,2) ) / (count-1) ); 
            standardD = Math.sqrt(varSq / (count-1));                           */
